...
void setup() {
  ...
  //rtc.writeSqwPinMode(SquareWave4kHz); // Used to simulate a RPM or SPEED signal, generate a 4kHz signal through the DS1338. Use a pullup R (10kohm).
    ...
    if (isRunning) { // If stopwatch is running ...
      ...
      if (trackFile) {
        while (trackFile.available()) {
          csvReadText(&trackFile, trackName, sizeof(trackName), CSV_DELIM); // One line per track : "CAROLE;48.979993;2.522435;48.980023;2.522633" (<trackname>;<strartline_a_lat>;<strartline_a_lon>;<strartline_b_lat>;<strartline_b_lon>)
      ...
      // We adjust RTC clock with GPS datetime (UTC only)
      if (fix_data.valid.time) {
        rtc.adjust(DateTime(fix_data.dateTime.year, fix_data.dateTime.month, fix_data.dateTime.date, fix_data.dateTime.hours, fix_data.dateTime.minutes, fix_data.dateTime.seconds));
      }
      ...
      if (logFile.open(filename, O_CREAT | O_WRITE | O_EXCL)) {
        logFile.println(F("lap;timestamp;brake;throttle;gear;rpm;speed;speedgps;heading;roll;pitch;temp;lat;lon"));
      } else {
        initerror();
      }
  ...
  if (isRunning) {
    if (gpsDataReady && sensorsDataReady) {
      ...
      if (addFinishLog == true) {
        ...
        DEBUG_PORT.print(".");
        DEBUG_PORT.println(fix_data.dateTime);*/
      }
      logFile.print(F(";"));
      logFile.print(brake);
      logFile.print(F(";"));
